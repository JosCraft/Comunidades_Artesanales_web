...
$(document).ready(function() {
            ...
            success: function(resp) {
                ...
                if (resp.discount > 0) { // if there's a discount    // this is the same as:    if (resp['discount'] > 0) {
                    $('.getAttributePrice').html(
                        '<div class="price"><h4>Bs.' + resp.final_price.toFixed(2) + '</h4></div><div class="original-price"><span>Original Price: </span><span>Bs.' + resp.product_price + '</span></div>'
                    ); // Note: resp.product_price    is the same as    resp['product_price']
                } else { // if there's no discount
                    $('.getAttributePrice').html(
                        '<div class="price"><h4>Bs.' + resp.final_price.toFixed(2) + '</h4></div>'
        ...
        $.ajax({
            ...
            success: function(resp) { // if the AJAX request is successful
                ...
                if (resp.couponAmount > 0) { // if there's a coupon code submitted and it's valid        // 'couponAmount' is sent as a PHP array key (in the HTTP response from the server (backend)) from inside the applyCoupon() method in Front/ProductsController.php
                    $('.couponAmount').text('Bs. ' + resp.couponAmount.toFixed(2)); // 'couponAmount' is sent as a PHP array key (in the HTTP response from the server (backend)) from inside the applyCoupon() method in Front/ProductsController.php    
                } else {
                    $('.couponAmount').text('Bs. 0');
                }
                ...
                if (resp.grand_total > 0) { // if there's a coupon code submitted and it's valid        // 'grand_total' is sent as a PHP array key (in the HTTP response from the server (backend)) from inside the applyCoupon() method in Front/ProductsController.php
                    $('.grand_total').text('Bs. ' + resp.grand_total.toFixed(2)); // 'grand_total' is sent as a PHP array key (in the HTTP response from the server (backend)) from inside the applyCoupon() method in Front/ProductsController.php    
                }
            },
            error  : function() { // if the AJAX request is unsuccessful
                alert('Error');
    ...
    $('input[name=address_id]').bind('change', function() {
        ...
        $('.shipping_charges').html('Bs. ' + shipping_charges.toFixed(2));
       ...
       // $('.couponAmount').html('Bs. ' + coupon_amount);
       $('.couponAmount').html('Bs. ' + coupon_amount.toFixed(2));
       ...
       // $('.grand_total').html('Bs. ' + grand_total);
        $('.couponAmount').html('Bs. ' + grand_total.toFixed(2));
